...
package body BPM_Calcul is
   ...
   function Get_BPM return Integer is
   begin
   ...
   begin
      Cardiac_Info.Signal = Read_Signal;
   end;

   procedure Set_Default_Thresh(val : Integer) is
   begin
      Cardiac_Info.Default_Thresh = val;
   end;


   procedure Process is
   begin
      Cardiac_Info.Sample_Counter += Cardiac_Info.Sample_Interval_Ms;
      int N = Cardiac_Info.Sample_Counter - Cardiac_Info.Last_Beat_Time;

      -- Find peak and through
      if Cardiac_Info.Signal < Cardiac_Info.Thresh and N > (IBI / 5) * 3 then
      ...
      if N > 250 then
         if Cardiac_Info.Signal > Cardiac_Info.Thresh and
           ...
           N > (Cardiac_Info.IBI / 5) * 3 then
            ...
            Cardiac_Info.BPM = 60000 / Cardiac_Info.IBI;
         end if;
      end if;
